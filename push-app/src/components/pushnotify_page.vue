      ...
      <v-col class="my-3" offset="1" cols="10">
        ...
        <v-card color="blue-grey lighten-1" dark>
          ...
          <v-container class="my-3">
            ...
            <v-row>
              <v-col cols="12">
                <v-text-field autofocus name="title" label="제목" type="text" v-model="sTitle" color="white"></v-text-field>
              </v-col>
              <v-col cols="12">
                <!-- rows 와 multi-line 어트피뷰트 사용으로 3줄 입력 컨트롤로 변경 -->
                <v-textarea rows="3" name="message" label="내용" type="text" v-model="sMsg" color="white"></v-textarea>
              </v-col>
            </v-row>
          </v-container>
          <v-card-actions>
            <!-- block 어트리뷰트 사용으로 아래쪽에 발송 블록 레벨 버튼 표시 -->
            <v-btn block large color="orange" dark @click="fnSendPush">
              <v-icon left>message</v-icon>발송
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
...
export default {
  ...
  methods: {
    fnSendPush() {
      ...
      // 파이어 스토어 함수 실행
      fetch('https://us-central1-pwa-push-95ce5.cloudfunctions.net/storePushData', {
        method: "POST",
        headers: {

...
export default {
  ...
  actions: {
    ...
    // 이메일 회원 로그인
    fnDoLogin({commit}, paylaod) {
      commit("fnSetLoading", true); // 스토어에 시간 걸림으로 상태 변경
      // 파이어베이스에 이메일 회원 로그인 인증 처리 요청
      firebase.auth()
              .signInWithEmailAndPassword(paylaod.pEmail, paylaod.pPassword)
              .then(pUserInfo => {
                // 로그인에 성공하면 스토어에 계정 정보 저장
                commit("fnSetUser", {
                  id: pUserInfo.uid,
                  namae: pUserInfo.displayName,
                  email: pUserInfo.email,

    ...
    <v-row>
      ...
      <v-col class="text-center" cols="8" offset="2" sm="6" offset-sm="3">
        ...
        <form @submit.prevent="fnRegisterUser">
          <!-- 필수 입력사항이 되도록 ㄱequired 어트리뷰트 적용 -->
...
export default {
  data() {
    return {
      ...
      sEmail: "",  // 이메일 입력 값 임시 저장
      sPassword: "",  // 비밀번호 입력 값 임시 저장
      sConfirmPassword: ""  // 비밀번호 입력 확인 값 임시 저장
    };
  },
  computed: {
    // 비밀번호 와 비밀번호 확인 값이 일치하는지 검사
    fnComparePassword() {
  ...
  watch: {
    // fnGetErrMsg() 함수가 오류 메시지를 반환하면 오류메시지 표시
    fnGetErrMsg(pMsg) {
      if (pMsg) this.bAlert = true;
    },
    // bAlert값이 false 로 바뀌면 에러 메시지 초기화
    bAlert(pValue) {
